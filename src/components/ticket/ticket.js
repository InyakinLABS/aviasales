  ...
  return (
    <div className={styles.ticket}>
      <div className={styles['ticket-header']}>
        ...
        <img className={styles['ticket-logo']} src={`//pics.avs.io/99/36/${carrier}.png`} alt="Логотип авиакомпании" />
      </div>

      <div className={styles['ticket-content']}>{segments.map((segment, index) => renderSegment(segment, index))}</div>
    </div>
  )
})
